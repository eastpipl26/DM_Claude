...
`공사일정.md` / `업무목록.md` / `인허가.md`에 마감일(마감일·처리기한)이 있는 행이 추가·변경되면, 해당 행을 구글캘린더 이벤트로 만든다.

- calendarId 매핑: `config.md`의 "구글캘린더" 표 참조 (phase/구분 → calendarId)
- "예시 행 — 실제 등록 후 삭제" 같은 placeholder 행은 동기화 대상에서 제외
- 이벤트 제목: `{항목/건명/제목}`, 종일 이벤트로 마감일에 생성, description에 "Obsidian {파일명} 연동 · 구분: {값} · 상태: {값}" 기록
- 같은 항목으로 이미 생성된 이벤트가 있으면(제목 fullText 검색) 중복 생성하지 말고 상태 변경 시 업데이트
- 표에서 행이 삭제되거나 상태가 "완료"로 바뀌면, 대응하는 캘린더 이벤트도 삭제하거나 완료 표시(제목에 ✅ 접두)
- 이 동기화는 파일 저장 직후 옥순이가 즉시 수행하거나, 야간 정리 루틴 1단계 이후에 일괄 수행
